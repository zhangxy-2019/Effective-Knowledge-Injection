...
from .reward_model import FSRewardModel
from ..utils import load_state_dict_into_model

def reload_hf_model(model_class,
                    model_name_or_path,
                    tokenizer,
                    ds_config=None,
                    rlhf_training=False,
    ...
    # model = model_class.from_pretrained(model_name_or_path, device_map = 'balanced_low_0', torch_dtype=torch.float16)
...
def create_hf_model(model_class,
    ...
    model_config = AutoConfig.from_pretrained(model_name_or_path)
    # if save_safetensors:
    # setattr(model_config, "tie_word_embeddings", False)  # safetensors does not allow shared weights
    if disable_dropout:
        model_config.dropout = 0.0
