...
class UnsupervisedmixedDataset(Dataset):
    ...
    def __init__(
        ...
        # df = [item for item in df if item["task_key"] == "reading_comprehension"] 
        # df = [item for item in df if item["task_key"] == "answer_w_docs"]
        
        print("len df: ", len(df)) # # len df:  300
        # if few_shot: 
        #     prompts = [item["five_shot_text"] for item in df]
        # else:
        prompts = [item["text"] for item in df]
        ...
        new_prompts = []
        for index, (mc_label, prompt) in enumerate(zip(mc_labels, prompts)):
            new_prompt = prompt.strip() + mc_label.strip()
        ...
        for i, prompt in enumerate(new_prompts):
            ...
            # print("prompt text: \n", prompt)
            if "\nAnswer:" in prompt and not "- Wikipedia>" in prompt:
                instruction_content = prompt.split("\nAnswer:")[:-1]
                instruction_content = "\nAnswer:".join(instruction_content) + "\nAnswer:"
                # print("instruction text: ", instruction_content)
