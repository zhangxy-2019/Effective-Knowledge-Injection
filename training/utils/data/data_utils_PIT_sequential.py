...
class UnsupervisedtfDataset(Dataset):
    ...
    def __init__(
        ...
        super(UnsupervisedtfDataset, self).__init__()
        df = df["train"]

        df = [item for item in df if item["task"] == "instruction_following"] 
        # df = [item for item in df if item["task_key"] == "answer_w_docs"]
        print("len df: ", len(df)) # # len df:  300
        prompts = [item["text"] for item in df]
        mc_labels = [item["output"] for item in df]
        # task_keyword = "Question:"
        # print("len prompts: ", len(prompts))
        queries = []
        for i, prompt_text in enumerate(prompts):
            # split_word = "\n\n" + task_keyword
            prompt = prompt_text.strip() + mc_labels[i].strip()
            # if few_shot: 
            #     while len(tokenizer.tokenize(prompt)) + 1 > max_seq_len: # bos, end token
            #         prompt_split = prompt.split(split_word)
            #         prompt_split.pop(1)
            #         prompt = split_word.join(prompt_split)
            # else:
            #     prompt = task_keyword + prompt.split(split_word)[-1]
            
            # print("prompt text: \n", prompt)
            instruction_content = prompt.split("\nAnswer:")[:-1]
            instruction_content = "\nAnswer:".join(instruction_content) + "\nAnswer:"
            # print("instruction text: ", instruction_content)

            instruction_prompt = torch.tensor(
                tokenizer.encode(instruction_content), dtype=torch.int64
            )
            # print("instruction prompt: ", instruction_prompt)
            # print("instr_prompt size: ", instruction_prompt.shape) # torch.Size([194])
            
            prompt_token = tokenizer(prompt, return_tensors="pt")
            for key_word in ["input_ids", "attention_mask"]:
                y = prompt_token[key_word][:, -max_seq_len:]
                prompt_token[key_word] = y
            # prompt_token = tokenizer(prompt, truncation=True, max_length=max_seq_len, padding="max_length", return_tensors="pt")

            ### padding
            length = prompt_token["input_ids"].size()[-1]
            pad_length = max_seq_len - length
            if pad_length > 0:
                prompt_token["input_ids"] = F.pad(prompt_token["input_ids"],
                                        pad=(0, pad_length),
                                        mode='constant',
                                        value=pad_token_id)
                prompt_token["attention_mask"] = F.pad(prompt_token["attention_mask"],
                                                pad=(0, pad_length),
                                                mode='constant',
                                                value=0)
                
            assert prompt_token["input_ids"].size()[-1] == prompt_token["attention_mask"].size()[-1]
            example = prompt_token["input_ids"]
            # print("Example shape is ", example.shape) # torch.Size([1, 4096])
            # print("example ids: ", example)

            labels = copy.deepcopy(example)
            # print("label ids: ", labels)
            # print("label ids afer ignorting shape: ", labels.shape)
            # print("len(instruction_prompt): ", len(instruction_prompt)) # len(instruction_prompt):  194
            labels[0, :len(instruction_prompt)] = IGNORE_INDEX
            # print("label ids after ignoring: ", labels)
            # print("label ids afer ignorting shape: ", labels.shape)
            # label ids:  tensor([[    1,  4124, 19819,  ...,     2,     2,     2]])
            # label ids afer ignorting shape:  torch.Size([1, 4096])
            # label ids after ignoring:  tensor([[-100, -100, -100,  ...,    2,    2,    2]])
            # label ids afer ignorting shape:  torch.Size([1, 4096])
            # example_mask = example_mask.float()
            # label_mask = label_mask.float()
            # prompt_token["input_ids"] = example
            # prompt_token["attention_mask"] = example_mask
            prompt_token["labels"] = labels
            # print("example ids: ", prompt_token["input_ids"])
            # print("example mask: ", prompt_token["attention_mask"])
            # print("example labels: ", prompt_token["labels"])
            # instr_prompt size:  torch.Size([194])
            # example ids:  tensor([[    1,  4124, 19819,  ...,     2,     2,     2]])
            # example mask:  tensor([[1, 1, 1,  ..., 0, 0, 0]])
            # example labels:  tensor([[-100, -100, -100,  ..., -100, -100, -100]])
            queries.append(prompt_token)


        query_ids = [query["input_ids"] for query in queries]
        query_attn_masks = [query["attention_mask"] for query in queries]
        ...
        self.query_attn_masks = query_attn_masks
    ...
    def __getitem__(self, i):
        return dict(queries=self.queries[i], query_attn_masks=self.query_attn_masks[i], query_labels=self.query_labels[i], pad_token_id=self.pad_token_id)

    def __len__(self):
        return len(self.queries)


class UnsupervisedmixedDataset(Dataset):
    """Dataset that emits tokenized left-padded queries."""
    ...
    def __init__(
        ...
        super(UnsupervisedmixedDataset, self).__init__()
        df = df["train"]

        df_ins = [item for item in df if item["task"] == "instruction_following"] 
        df_doc = [item for item in df if item["task"] == "pure_evidence"]
        
        # print("len df: ", len(df)) # # len df:  300
        assert len(df_ins) == len(df_doc)
        prompts = [item["text"] for item in df]
            
        mc_labels = [item["output"] for item in df]
        # mc_labels = [ins_label.split("\nSupporting Documents:")[0] for ins_label in mc_labels]

        new_prompts = []
        for index, (ins, doc) in enumerate(zip(df_ins, df_doc)):
            new_prompt = ins["text"].strip() + ins["output"].strip()
            new_prompts.append(new_prompt)
            new_prompts.append(doc["output"])


        # task_keyword = "Question:"
        # bio_texts = [item["output"] for item in df if item["task_key"] == "instruction_bio"]
        # prompts = new_prompts + bio_texts
...
def create_prompt_dataset(local_rank,
    ...
    fname = f"{fname}_phase{train_phase}_seed{seed}_tokenizer{tokenizer_name}_seqlen{max_seq_len}_sft{sft_cache_key}_stable"
    ...
    if local_rank <= 0 and (buf_create_cache.item() != 0 or reload):
        ...
        if stage == 1:
            train_dataset = UnsupervisedtfDataset(
                df=train_df,
                tokenizer=tokenizer,
            ...
            train_dataset = UnsupervisedmixedDataset(
                df=train_df,
                tokenizer=tokenizer,
                max_seq_len=max_seq_len,
                pad_token_id=tokenizer.pad_token_id,
        ...
        # eval_dataset = UnsupervisedmixedDataset(
        #     df=train_df,
        #     tokenizer=tokenizer,
        #     max_seq_len=max_seq_len,
        #     pad_token_id=tokenizer.pad_token_id,
...
class DataCollatorRLHF:
    ...
    def __call__(self, data):
        batch = {}
        # prompt_tf = [f["tf_queries"] for f in data]
        # prompt_ans = [f["ans_list"] for f in data]
        prompt_mclabel = [f["mc_label"] for f in data]
        ...
        #                                      value=0)
        # else:
        batch["prompt"] = torch.tensor([f["queries"].numpy().tolist()[0] for f in data])
        # batch["prompt_att_mask"] = prompt_mask
        # batch["prompt_text"] = [f["prompts"] for f in data]
        batch["prompt_att_mask"] = torch.tensor([f["query_attn_masks"].numpy().tolist()[0] for f in data])
        
        # batch["prompt_tf"] = prompt_tf
        # batch["prompt_ans"] = prompt_ans
        batch["prompt_mclabel"] = prompt_mclabel
        # batch["prompt_mclabel"] = prompt_mclabel
...
class DataCollatorUnsupervised:
    ...
    def __call__(self, data):
        ...
        batch["input_ids"] = torch.tensor([f["queries"].numpy().tolist()[0] for f in data])
        batch["labels"] = torch.tensor([f["query_labels"].numpy().tolist()[0] for f in data])
        batch["attention_mask"] = torch.tensor([f["query_attn_masks"].numpy().tolist()[0] for f in data])
        # print("batch data unsupervised:\n", batch)
        return batch
